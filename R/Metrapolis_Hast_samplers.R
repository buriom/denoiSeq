...
MH_p <- function(N, f, p_b, propotns, x, step_size) {
    ...
    r <- exp(fullcond_p(N, f, p_c, propotns, x) - fullcond_p(N, f, p_b, propotns, x) + dbeta(p_b, step_size * p_c, 
        step_size * (1 - p_c), log = TRUE) - dbeta(p_c, step_size * p_b, step_size * (1 - p_b), log = TRUE))
    if (u < r) {
        p_a <- p_c
    } else {
        p_a <- p_b
...
MH_f <- function(N, f_b, p, propotns, x, step_size) {
    ...
    r <- exp(fullcond_f(N, f_c, p, propotns, x) - fullcond_f(N, f_b, p, propotns, x) + dbeta(f_b, step_size * f_c, 
        step_size * (1 - f_c), log = TRUE) - dbeta(f_c, step_size * f_b, step_size * (1 - f_b), log = TRUE))
    if (u < r) {
        f_a <- f_c
    } else {
        f_a <- f_b
...
# _____________________________________________________________________________ To implement
# Metrapolis-Hastings for N
