...
MH_N <- function(N_b, f, p, propotns, x, step_size) {
    ...
    u1 <- k[1]
    u2 <- k[2]
    N_c <- exp(log(N_b) + step_size * 2 * (u1 -
        1/2))
...
MH_p <- function(N, f, p_b, propotns, x, step_size) {
    u <- runif(1, 0, 1)
    p_c <- rbeta(1, step_size * p_b, step_size *
        (1 - p_b))
    r <- exp(fullcond_p(N, f, p_c, propotns, x) -
        fullcond_p(N, f, p_b, propotns, x) + dbeta(p_b,
        step_size * p_c, step_size * (1 - p_c),
        log = TRUE) - dbeta(p_c, step_size * p_b,
...
MH_f <- function(N, f_b, p, propotns, x, step_size) {
    u <- runif(1, 0, 1)
    f_c <- rbeta(1, step_size * f_b, step_size *
        (1 - f_b))
    r <- exp(fullcond_f(N, f_c, p, propotns, x) -
        fullcond_f(N, f_b, p, propotns, x) + dbeta(f_b,
        step_size * f_c, step_size * (1 - f_c),
        log = TRUE) - dbeta(f_c, step_size * f_b,
