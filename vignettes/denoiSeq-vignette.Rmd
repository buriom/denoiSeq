...
```
simdat <- simdat[ rowSums(simdat) > 15, ]
```
to eliminate the lowly expressed genes  since as has been reported [@bullard2010evaluation], it is difficult to extract a signal of difference  in these kinds of genes. A threshold of 15 produced the highest AUC (0.847) for an annonated real dataset, hence the choice.
...
A threshold value of the statistic is then chosen to determine significance. This should be close to 0.35 or determined by one arranging the stat column in ascending order and selecting the most differentially expressed genes.
```
