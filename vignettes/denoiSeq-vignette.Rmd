...
The model generates in a bottom up manner, a probability distribution of final gene
copy number, which is a superposition of the negative binomial and the binomial distributions.

The denoiSeq package is based on a  model for PCR  sequencing developed by  @ndifon2012chromatin, which generates in a bottom up manner, a probability distribution of final gene
copy number, that is a superposition of the negative binomial and the binomial distributions.
...
```
to eliminate the lowly expressed genes  since as has been reported [@bullard2010evaluation], it is difficult to extract a signal of difference  in these kinds of genes. Threshold values of 30 for large datasets (>400 genes) and 15 for smaller datasets (<=100 genes) were chosen based on runs using real datasets as shown in the main text for this model.
...
###Pre-filtering
We pre-filtered the dataset by selecting rows with a rowsums above 15. This value represents averagely, a  slightly above 1 counts in each column. This threshold is motivated by the fact that the highest AUC (0.847) was  obtained from prior analysis of the same dataset, using a significance  cut off of stat equivalent to 0.38.
```{r, echo=TRUE}
...
A threshold value of the statistic is then chosen to determine significance. One can be determined by one arranging the stat column in ascending order and selecting the most differentially expressed genes. Values ranging from 0.003 to 0.23 were obtained for the Youden's index for several annotated real datasetsas shown in the main text.
```
